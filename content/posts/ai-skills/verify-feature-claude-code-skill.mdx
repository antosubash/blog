...
### Stage 0 — Commit pending changes

The pipeline assumes a clean working tree. If files are still uncommitted when the skill starts, the PR at the end will be missing them — and stage 4's "production build" tells you nothing about the version you actually meant to ship.

The skill resolves this up front: stage and commit anything pending, then move on.
...
- **One commit, descriptive message.** Squash the pending changes into a single commit whose message follows the repo's existing style (read `git log` first). This is the commit that will appear in the PR.
- **Never amend.** If a previous commit on the branch already exists, create a new commit on top — amending rewrites history the user may have already pushed.

If the working tree is already clean, this stage is a no-op and the skill drops straight into stage 1.
...
**`gh pr create` fails with `not authenticated`.** The skill assumes `gh auth status` is green. If you're running on a fresh machine, the skill should detect this in stage 5 and stop with a clear "run `gh auth login` first" instead of silently failing. Add a pre-flight `gh auth status` check to stage 5 if your team hits this often.
...
The pipeline assumes a clean working tree. If anything is uncommitted, the PR
at the end will be missing those files. Commit before continuing.
...
- Inspect the diff before committing. If `.env`, credentials, large binaries,
  or scratch files appear, **stop and ask the user** — do not stage them.
- Create a new commit; never `--amend` an existing one.
- If the working tree is already clean, this stage is a no-op.
