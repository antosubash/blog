...
export function getFiles(type: any) {
  const prefixPaths = path.join(root, "_posts", type);
...
export async function getFileBySlug(type: any, slug: any) {
  const mdxPath = path.join(root, "_posts", type, `${slug}.mdx`);
  const mdPath = path.join(root, "_posts", type, `${slug}.md`);
...
export async function getAllFilesFrontMatter(folder: any) {
  const prefixPaths = path.join(root, "_posts", folder);

  const files = getAllFilesRecursively(prefixPaths);

  const allFrontMatter = [] as any;
